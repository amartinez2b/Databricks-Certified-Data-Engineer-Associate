...
df1.agg(min("_id")).collect()[0][0], df1.agg(max("ID")).collect()[0][0]
...
df2.agg(min("ID")).collect()[0][0], df2.agg(max("_id")).collect()[0][0]
...
df1 = df1.withColumn("Score1_double", col("Score01") * 2)
...
df1.createOrReplaceTemporalView("table1")

# Register 'df2' as a SQL table
df2.createOrReplaceTemporalView("table2")
